#!/usr/bin/env python3
"""
Script de configuração inicial do CMS Agência Kaizen
Cria páginas básicas e configura o site
"""

import os
import sys
from pathlib import Path

# Adicionar o diretório src ao path
sys.path.insert(0, str(Path(__file__).parent.parent / 'src'))

# Configurar Django
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'agenciakaizen_cms.settings.dev')
import django
django.setup()

from django.contrib.auth import get_user_model
from wagtail.models import Page, Site
from home.models import HomePage
from blog.models import BlogIndexPage, BlogCategory
from portfolio.models import PortfolioIndexPage, PortfolioCategory
from services.models import ServicesIndexPage
from contact.models import ContactPage, ContactFormPage


def create_superuser():
    """
    Criar superusuário se não existir
    """
    User = get_user_model()
    
    if not User.objects.filter(username='admin').exists():
        User.objects.create_superuser(
            username='admin',
            email='admin@www.agenciakaizen.com.br',
            password='admin123'
        )
        print("✅ Superusuário 'admin' criado (senha: admin123)")
    else:
        print("ℹ️  Superusuário 'admin' já existe")


def create_site():
    """
    Configurar site principal
    """
    # Buscar ou criar home page
    try:
        home_page = HomePage.objects.get(slug='home')
    except HomePage.DoesNotExist:
        # Criar home page
        home_page = HomePage(
            title="Agência Kaizen",
            slug="home",
            hero_title="Transformamos Ideias em Soluções Digitais",
            hero_subtitle="<p class='lead'>Somos uma agência digital especializada em criar experiências únicas que conectam marcas e pessoas através da tecnologia.</p>",
            about_title="Sobre a Agência Kaizen",
            about_text="<p class='lead'>Somos uma equipe apaixonada por tecnologia e inovação, dedicada a transformar ideias em soluções digitais que fazem a diferença.</p><p>Com anos de experiência no mercado, oferecemos serviços completos de desenvolvimento web, marketing digital e design, sempre focados em resultados excepcionais para nossos clientes.</p>",
            services_title="Nossos Serviços",
            services_text="<p class='lead'>Oferecemos soluções completas para impulsionar seu negócio no mundo digital.</p>",
            portfolio_title="Nosso Portfolio",
            portfolio_text="<p class='lead'>Conheça alguns dos projetos que desenvolvemos para nossos clientes.</p>",
            contact_title="Pronto para Começar?",
            contact_text="<p class='lead'>Entre em contato conosco e vamos transformar sua ideia em realidade.</p>"
        )
        
        # Adicionar como filha da root page
        root = Page.get_first_root_node()
        root.add_child(instance=home_page)
        home_page.save()
        print("✅ Home page criada")
    else:
        print("ℹ️  Home page já existe")
    
    # Configurar site
    site = Site.objects.first()
    if site:
        site.root_page = home_page
        site.hostname = 'www.agenciakaizen.com.br'
        site.port = 80
        site.is_default_site = True
        site.save()
        print("✅ Site configurado")
    else:
        site = Site.objects.create(
            hostname='www.agenciakaizen.com.br',
            port=80,
            root_page=home_page,
            is_default_site=True
        )
        print("✅ Site criado e configurado")


def create_blog_pages():
    """
    Criar páginas do blog
    """
    try:
        blog_index = BlogIndexPage.objects.get(slug='blog')
        print("ℹ️  Página do blog já existe")
    except BlogIndexPage.DoesNotExist:
        home_page = HomePage.objects.first()
        if not home_page:
            print("❌ Home page não encontrada")
            return
        
        blog_index = BlogIndexPage(
            title="Blog",
            slug="blog",
            intro="<p>Artigos, dicas e insights sobre desenvolvimento web, marketing digital e tecnologia.</p>"
        )
        home_page.add_child(instance=blog_index)
        blog_index.save()
        print("✅ Página do blog criada")
    
    # Criar categorias do blog
    categories = [
        {'name': 'Desenvolvimento Web', 'slug': 'desenvolvimento-web'},
        {'name': 'Marketing Digital', 'slug': 'marketing-digital'},
        {'name': 'Design', 'slug': 'design'},
        {'name': 'SEO', 'slug': 'seo'},
        {'name': 'Tecnologia', 'slug': 'tecnologia'}
    ]
    
    for cat_data in categories:
        category, created = BlogCategory.objects.get_or_create(
            slug=cat_data['slug'],
            defaults={'name': cat_data['name']}
        )
        if created:
            print(f"✅ Categoria criada: {category.name}")


def create_portfolio_pages():
    """
    Criar páginas do portfolio
    """
    try:
        portfolio_index = PortfolioIndexPage.objects.get(slug='portfolio')
        print("ℹ️  Página do portfolio já existe")
    except PortfolioIndexPage.DoesNotExist:
        home_page = HomePage.objects.first()
        if not home_page:
            print("❌ Home page não encontrada")
            return
        
        portfolio_index = PortfolioIndexPage(
            title="Portfolio",
            slug="portfolio",
            intro="<p>Conheça alguns dos projetos que desenvolvemos para nossos clientes.</p>"
        )
        home_page.add_child(instance=portfolio_index)
        portfolio_index.save()
        print("✅ Página do portfolio criada")
    
    # Criar categorias do portfolio
    categories = [
        {'name': 'Sites Institucionais', 'slug': 'sites-institucionais'},
        {'name': 'E-commerce', 'slug': 'e-commerce'},
        {'name': 'Aplicações Web', 'slug': 'aplicacoes-web'},
        {'name': 'Landing Pages', 'slug': 'landing-pages'}
    ]
    
    for cat_data in categories:
        category, created = PortfolioCategory.objects.get_or_create(
            slug=cat_data['slug'],
            defaults={'name': cat_data['name']}
        )
        if created:
            print(f"✅ Categoria do portfolio criada: {category.name}")


def create_services_pages():
    """
    Criar páginas de serviços
    """
    try:
        services_index = ServicesIndexPage.objects.get(slug='servicos')
        print("ℹ️  Página de serviços já existe")
    except ServicesIndexPage.DoesNotExist:
        home_page = HomePage.objects.first()
        if not home_page:
            print("❌ Home page não encontrada")
            return
        
        services_index = ServicesIndexPage(
            title="Serviços",
            slug="servicos",
            intro="<p>Oferecemos soluções completas para impulsionar seu negócio no mundo digital.</p>"
        )
        home_page.add_child(instance=services_index)
        services_index.save()
        print("✅ Página de serviços criada")


def create_contact_pages():
    """
    Criar páginas de contato
    """
    try:
        contact_page = ContactPage.objects.get(slug='contato')
        print("ℹ️  Página de contato já existe")
    except ContactPage.DoesNotExist:
        home_page = HomePage.objects.first()
        if not home_page:
            print("❌ Home page não encontrada")
            return
        
        contact_page = ContactPage(
            title="Contato",
            slug="contato",
            intro="<p>Entre em contato conosco e vamos transformar sua ideia em realidade.</p>",
            address="<p>São Paulo, SP - Brasil</p>",
            phone="+55 11 99999-9999",
            email="contato@www.agenciakaizen.com.br"
        )
        home_page.add_child(instance=contact_page)
        contact_page.save()
        print("✅ Página de contato criada")


def create_sample_content():
    """
    Criar conteúdo de exemplo
    """
    print("Criando conteúdo de exemplo...")
    
    # Criar posts de exemplo do blog
    blog_index = BlogIndexPage.objects.first()
    if blog_index and not BlogPage.objects.exists():
        from blog.models import BlogPage
        from datetime import date
        
        sample_posts = [
            {
                'title': 'Como criar um site responsivo em 2024',
                'slug': 'como-criar-site-responsivo-2024',
                'intro': 'Aprenda as melhores práticas para desenvolver sites que funcionam perfeitamente em todos os dispositivos.',
                'body': '<p>O design responsivo é essencial nos dias de hoje. Com a crescente diversidade de dispositivos, é fundamental que seu site ofereça uma experiência consistente e otimizada para todos os usuários.</p><p>Neste artigo, vamos explorar as principais técnicas e ferramentas para criar sites verdadeiramente responsivos.</p>',
                'date': date(2024, 1, 15)
            },
            {
                'title': 'Marketing Digital: Tendências para 2024',
                'slug': 'marketing-digital-tendencias-2024',
                'intro': 'Descubra as principais tendências do marketing digital que vão dominar 2024.',
                'body': '<p>O marketing digital está em constante evolução. A cada ano, novas tecnologias e estratégias surgem, transformando a forma como as empresas se conectam com seus clientes.</p><p>Vamos analisar as tendências mais promissoras para 2024.</p>',
                'date': date(2024, 1, 10)
            }
        ]
        
        for post_data in sample_posts:
            post = BlogPage(
                title=post_data['title'],
                slug=post_data['slug'],
                intro=post_data['intro'],
                body=post_data['body'],
                date=post_data['date'],
                live=True,
                show_in_menus=False
            )
            blog_index.add_child(instance=post)
            post.save()
            print(f"✅ Post criado: {post.title}")


def main():
    """
    Função principal
    """
    print("🚀 Configurando CMS Agência Kaizen...")
    print("=" * 50)
    
    try:
        create_superuser()
        create_site()
        create_blog_pages()
        create_portfolio_pages()
        create_services_pages()
        create_contact_pages()
        create_sample_content()
        
        print("=" * 50)
        print("✅ Configuração concluída com sucesso!")
        print("\n📋 Próximos passos:")
        print("1. Acesse o admin: http://localhost:8000/admin/")
        print("2. Login: admin / Senha: admin123")
        print("3. Configure suas páginas e conteúdo")
        print("4. Execute: python manage.py runserver")
        
    except Exception as e:
        print(f"❌ Erro durante a configuração: {e}")
        sys.exit(1)


if __name__ == '__main__':
    main()
